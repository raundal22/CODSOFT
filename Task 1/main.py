...
class GUI:
  ...
  def __init__(self):
    ...
    self.tasks_overview = tk.Listbox(master=self.main_window,
                                     height=18,
                                     width=40)
    self.tasks_overview.grid(row=3,
                             column=0,
                             sticky="news",
                             columnspan=3,
                             pady=(20, 0))
  ...
  def update_overview(self):
    ...
    if self.selected_date not in [None, ""]:
      for task in self.task_list_dict[self.selected_date]:
        self.tasks_overview.insert(
            self.task_list_dict[self.selected_date].index(task), task)
  ...
  def listbox_used(self, selection):
    ...
    if self.task_listbox.curselection():
      self.selected_date = self.task_listbox.get(
